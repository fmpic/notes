...
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | --------------------------------- |
| `lmcache`     | [`pyproject.toml`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/pyproject.toml) + [`setup.py`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/setup.py) | common + 设备 profile requirements | 完整 runtime、sdist、native wheel |
| `lmcache-cli` | [`pyproject_cli.toml`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/pyproject_cli.toml)                                                                   | `requirements/cli.txt`             | 无 torch/native 的远程 CLI wheel  |

两者都由 setuptools-scm 生成版本；完整包通过 [`BuildPolicy`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/setup_extensions/policy.py#L127) — `setup_extensions/policy.py:127` 决定原生扩展。
...
定义：[`COMMON_EXTENSIONS`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/setup_extensions/common_cpp.py#L29) — `setup_extensions/common_cpp.py:29`。
...
源码：[`CudaProfile`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/setup_extensions/build_profiles/cuda.py#L23) — `setup_extensions/build_profiles/cuda.py:23`、[`RocmProfile`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/setup_extensions/build_profiles/rocm.py#L67) — `setup_extensions/build_profiles/rocm.py:67`、[`SyclProfile`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/setup_extensions/build_profiles/sycl.py#L21) — `setup_extensions/build_profiles/sycl.py:21`、[`MusaProfile`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/setup_extensions/build_profiles/musa.py#L19) — `setup_extensions/build_profiles/musa.py:19`。

### 已发现的构建缺口

[`SyclProfile.requirements_file`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/setup_extensions/build_profiles/sycl.py#L91) — `setup_extensions/build_profiles/sycl.py:91` 返回 `xpu_core.txt`，但该修订的 [`requirements/`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/requirements/) 没有这个文件；[`_read_requirements missing-file branch`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/setup.py#L27) — `setup.py:27` 会静默得到空列表。SYCL 用户需手工准备 XPU torch/runtime/oneAPI 依赖。
...
多个 storage profiles 可同时启用；设备 profile 只能选一个。入口：[`collect_storage_backends`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/setup_extensions/policy.py#L232) — `setup_extensions/policy.py:232`。

## Rust raw-block

[`rust/raw_block/pyproject.toml`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/rust/raw_block/pyproject.toml) 是独立 maturin/PyO3 项目，生成 `lmcache_rust_raw_block_io` cdylib。它供 legacy `RustRawBlockBackend` 与 MP `RawBlockL2Adapter` 共用；Python 负责 slot/checkpoint/task orchestration，Rust 只负责 raw I/O。
...
说明入口：[`docker/README.md`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/docker/README.md)。
...
发布图的权威文件：[`build_main_artifacts.yml`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/.github/workflows/build_main_artifacts.yml)、[`publish.yml`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/.github/workflows/publish.yml)、[`nightly_build.yml`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/.github/workflows/nightly_build.yml)、[`operator_release.yml`](https://github.com/zhiim/LMCache/blob/v0.5.0-dev/.github/workflows/operator_release.yml)。
